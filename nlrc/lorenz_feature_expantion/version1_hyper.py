...
for n in n_lst:
    for reg in reg_lst:
        ...
        y_test_testing = scalar.inverse_transform(y_test).copy()

        print(f'n {n} reg {reg} mse {mean_squared_error(y_test_testing,y_pred)}')
        mse_lst[(n,reg)] = mean_squared_error(y_test_testing,y_pred)
